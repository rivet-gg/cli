...
impl Opts {
	pub async fn execute(&self, ctx: &cli_core::Ctx) -> GlobalResult<()> {
		...
		// TODO: Add link to error message
		let project = unwrap!(
		...
		apis::ee_cloud_opengb_projects_envs_api::ee_cloud_opengb_projects_envs_provision_databases(
			...
			&project_id,
			&env_id,
			models::EeCloudOpengbProjectsEnvsProvisionDatabasesRequest { modules: modules.clone() },
		)
		.await?;
		...
		let api = format!(
			...
			project.name_id,
			env.name_id,
			ctx.bootstrap.domains.as_ref().unwrap().main
		);

		eprintln!("");
		rivet_term::status::success(
			"Done",
			format!("OpenGB API available at {}", term::link(api)),
		);
